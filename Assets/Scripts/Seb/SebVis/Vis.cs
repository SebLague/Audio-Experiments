...
namespace Seb.Visualization
{
    ...
    public static partial class Vis
    {
        ...
        public static void Text(FontType font, string text, float fontSize, Vector2 pos, Anchor anchor, Color col, float lineSpacing = 1, float angle = 0)
        {
            if (fontSize <= 0 || col.a == 0 || string.IsNullOrEmpty(text)) return;

            FontData fontData = defaultFontsData[(int)font];
            TextDrawData data = new(fontData, text, fontSize, lineSpacing, pos, anchor, col, activeMaskMin, activeMaskMax, angle);
            textDrawer.AddToLayer(data);
        }

        public static void Text(FontType font, char[] text, int textLength, float fontSize, Vector2 pos, Anchor anchor, Color col, float lineSpacing = 1, float angle = 0)
        {
            if (fontSize <= 0 || col.a == 0 || text == null || text.Length == 0 || textLength == 0) return;

            FontData fontData = defaultFontsData[(int)font];
            TextDrawData data = new(fontData, text, textLength, fontSize, lineSpacing, pos, anchor, col, activeMaskMin, activeMaskMax, angle);
            textDrawer.AddToLayer(data);
        }
        ...
        public static void LineRay(Vector2 origin, Vector2 offset, float thickness, Color col, float t = 1)
        {
        ...
        public static void LinePath(List<Vector2> points, float thickness, Color col, float animT = 1)
        {
        ...
        public static void LinePath(Span<Vector2> points, float thickness, Color col, float animT = 1)
        {
            if (col.a == 0 || thickness == 0 || animT <= 0) return;
        ...
        public static void DataLine(Span<float> yVals, float xStart, float xEnd, float yMul, float yOffset, float thickness, Color col, float animT = 1)
        {
            if (thickness == 0 || col.a == 0 || animT <= 0) return;

            Vector2 posPrev = GetPoint(yVals, 0);

            for (int i = 1; i < yVals.Length; i++)
            {
                float t = i / (yVals.Length - 1f);
                Vector2 posNext = GetPoint(yVals, i);

                if (t > animT)
                {
                    float tPrev = (i - 1f) / (yVals.Length - 1f);
                    float localT = Mathf.InverseLerp(tPrev, t, animT);
                    posNext = Vector2.Lerp(posPrev, posNext, localT);
                }

                Line(posPrev, posNext, thickness, col);
                posPrev = posNext;
                if (t > animT) break;
            }
            ...
            Vector2 GetPoint(Span<float> yVals, int i)
            {
                float y = (float)(yVals[i] * yMul + yOffset);
                float t = i / (yVals.Length - 1f);
        ...
        public static void DataLine(double[] yVals, float xStart, float xEnd, float yMul, float yOffset, float thickness, Color col, float animT = 1)
        {
            Span<float> yValsF = stackalloc float[yVals.Length];
            for (int i = 0; i < yVals.Length; i++)
            {
                yValsF[i] = (float)yVals[i];
            }

            DataLine(yValsF, xStart, xEnd, yMul, yOffset, thickness, col, animT);
        }
