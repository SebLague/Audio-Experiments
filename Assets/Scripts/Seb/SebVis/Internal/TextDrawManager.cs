...
namespace Seb.Visualization
{
	public class TextDrawManager : Drawer<TextDrawData>
	{
		...
		protected override void DrawLayer(CommandBuffer cmd, int startIndex, int count, Vis.LayerInfo layerInfo)
		{
			if (count == 0) return;
			...
			for (int i = startIndex; i < startIndex + count; i++)
			{
				...
				renderer.AddTextGroup(text, data.fontData, layoutSettings, pos, data.col, data.angle, layerInfo.useScreenSpace, maskMin, maskMax, data.anchor);
			}

			renderer.Render(cmd);
		}

		protected override void OnLayerAdded(Vis.LayerInfo layerInfo)
		{
