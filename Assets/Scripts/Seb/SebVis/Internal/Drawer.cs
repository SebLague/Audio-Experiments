...
namespace Seb.Visualization.Internal
{
	public abstract class Drawer<T> where T : struct
	{
		...
		readonly List<Vis.LayerInfo> layers = new();
		...
		public void StartNewLayer(Vis.LayerInfo layerInfo)
		{
		...
		public void DrawNextLayer(CommandBuffer cmd)
		{
			...
			Vis.LayerInfo layerInfo = layers[layerDrawIndex];
			if (count > 0) DrawLayer(cmd, startIndex, count, layerInfo);
			layerDrawIndex++;
			startIndex += count;
		}

		protected abstract void DrawLayer(CommandBuffer cmd, int startIndex, int count, Vis.LayerInfo layerInfo);

		protected abstract void InitFrame();

		protected virtual void OnLayerAdded(Vis.LayerInfo layerInfo)
		{
		}
