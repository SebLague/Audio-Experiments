...
using Seb.Visualization.Text.Rendering.Helpers;
using UnityEngine;
...
namespace Seb.Visualization.Text.Rendering
{
	public class TextRenderer
	{
		...
		public void AddTextGroup(ReadOnlySpan<char> text, FontData fontData, LayoutSettings settings, Vector2 pos, Color textCol, float angle, bool useScreenspace, Vector2 maskMin, Vector2 maskMax, Anchor anchor)
		{
			InitFrame();
			...
			TextGroup groupData = new(worldOffset, maskMin, maskMax, angle, useScreenspace ? 1 : 0);
		...
		public readonly struct TextGroup
		{
			...
			public TextGroup(Vector2 offset, Vector2 maskMin, Vector2 maskMax, float angle, int useScreenSpace)
			{
				Offset = offset;
				MaskMin = maskMin;
				MaskMax = maskMax;
