    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v, uint instanceID : SV_InstanceID)
            {
                ...
                float2 worldVertPos = v.vertex * quadSize * instance.fontSize;

                if (group.angle != 0)
                {
                    float angle = group.angle;
                    float2 ihat = float2(cos(angle), sin(angle));
                    float2 jhat = float2(-ihat.y, ihat.x);
                    worldVertPos = ihat * worldVertPos.x + jhat * worldVertPos.y;
                }
                worldVertPos += offset;
            ...
            float4 frag(v2f input) : SV_Target
            {
                ...
                float pixelSize = abs(ddx(input.pos.x));
                float alphaSum = 0;
