...
public class MyVisTest : MonoBehaviour
{
	...
	void DrawSceneObjects()
	{
		...
		Vis.Quad(screenSize / 2, screenSize - Vector2.one * 10, new Color(0.1f, 0.1f, 0.1f, 1));
		// Draw.EndTransformState();

		Vis.StartLayer(offsetTest, scaleTest, false);
		...
		for (int i = 0; i < transform.childCount; i++)
		{
			...
			if (t.TryGetComponent(out PointVisTest p))
			{
				...
				Vis.Ellipse(p.transform.position, p.transform.localScale, p.Col);
			}
			else if (t.TryGetComponent(out QuadVisTest q))
			{
				if (q.outline) Vis.QuadOutline(q.transform.position, q.transform.localScale, q.thickness, q.col);
				else Vis.Quad(q.transform.position, q.transform.localScale, q.col);
			}
			else if (t.TryGetComponent(out LineVisTest l))
			{
				Vis.Line(l.transform.position, l.transform.position + (Vector3)l.offset, l.thickness, l.col, l.t);
			}
			...
			{
				Vis.Text(tv.fontType, tv.text, tv.fontSize, tv.transform.position, tv.textAlign, tv.col);
			}
			else if (t.TryGetComponent(out PathVisTest path))
			{
				Vis.LinePath(path.GetComponentsInChildren<Transform>().Select(t => (Vector2)t.position).ToArray(), path.thickness, path.col, path.t);
			}
			else if (t.TryGetComponent(out ArrowVisTest a))
			{
				if (a.useHeadDefaults)
				{
					Vis.Arrow(a.transform.position, (Vector2)a.transform.position + a.offset, a.thickness, a.col, a.t);
				}
				else
				{
					Vis.Arrow(a.transform.position, (Vector2)a.transform.position + a.offset, a.thickness, a.headLength, a.headAngle, a.col, a.t);
				}
			}
			else if (t.TryGetComponent(out TriangleVisTest tri))
			{
				Vector2 c = tri.transform.position;
				Vis.Triangle(c + tri.offsetA, c + tri.offsetB, c + tri.offsetC, tri.col);
			}
		}
