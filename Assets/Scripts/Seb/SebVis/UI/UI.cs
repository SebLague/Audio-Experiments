...
using UnityEngine;
using static Seb.Visualization.UI.UILayoutHelper;
...
namespace Seb.Visualization.UI
{
    public static class UI
    {
        ...
        static bool MouseInBounds(Vector2 centre, Vector2 size)
        {
            ...
            Vector2 offset = GetMousePos() - centre;
        ...
        public static UIScope CreateWorldSpaceUIScope(Vector2 pos)
        {
            UIScope scope = uiScopes.CreateScope();
            scope.canvasBottomLeft = pos;
            scope.canvasSize = Vector2.zero;
        ...
        public static UIScope CreateUIScope(Vector2 bottomLeft, Vector2 size, bool drawLetterboxes)
        {
            ...
            Vis.StartLayer(Vector2.zero, 1, true);
        ...
        public static Vis.MaskScope CreateMaskScopeMinMax(Vector2 maskMin, Vector2 maskMax)
        {
            ...
            (Vector2 parentMin_ss, Vector2 parentMax_ss) = Vis.GetActiveMaskMinMax();
            maskMin_ss = Vector2.Max(maskMin_ss, parentMin_ss);
            maskMax_ss = Vector2.Min(maskMax_ss, parentMax_ss);

            return Vis.BeginMaskScope(maskMin_ss, maskMax_ss);
        }

        public static Vis.MaskScope CreateMaskScope(Vector2 centre, Vector2 size) => CreateMaskScopeMinMax(centre - size / 2, centre + size / 2);

        public static Vis.MaskScope CreateMaskScope(Bounds2D bounds) => CreateMaskScopeMinMax(bounds.Min, bounds.Max);
        ...
        public static void DrawSlider(Vector2 pos, Vector2 size, Anchor anchor, ref SliderState state)
        {
            ...
            const float v = 0.15f;
            Vis.Quad(ss.centre, ss.size, new Color(v, v, v));
            ...
            if (state.handleSelected)
            {
                ...
                state.progressT = (GetMousePos().x - minX) / (maxX - minX);
            }

            const float w = 0.8f;
            const float v2 = 0.35f;
            float leftX = ss.centre.x - ss.size.x / 2;
            Vis.Quad((handlePos_ss + new Vector2(leftX, ss.centre.y)) / 2, new Vector2(handlePos_ss.x - leftX, ss.size.y), new Color(v2, v2, v2));
            Vis.Point(handlePos_ss, handleSize_ss, mouseOverHandle || state.handleSelected ? new Color(1, 0.22f, 0.32f) : new Color(w, w, w));
        ...
        public static ScrollBarState DrawScrollbar(Bounds2D scrollViewArea, Bounds2D scrollbarArea, float contentHeight, ScrollViewTheme theme, UIHandle id)
        {
            ...
            if (IsRendering)
            {
                ...
                Vis.Quad(barArea_ss.centre, barArea_ss.size, theme.scrollBarColBackground);
                ...
                // -- Handle input --
                bool mouseOverScrollbarArea = MouseInBounds(barArea_ss.centre, barArea_ss.size);
                bool mouseOverScrollbar = MouseInBounds(scrollbar_ss.centre, scrollbar_ss.size);
                ...
                if (mouseOverScrollbarArea && contentOverflow > 1)
                {
                    ...
                    if (InputHelper.IsMouseDownThisFrame(MouseButton.Left))
                    {
                        ...
                        state.dragScrollOffset = mouseOverScrollbar ? GetMousePos().y - scrollbar_ss.centre.y : 0;
                    }
                }
                ...
                if (state.isDragging)
                {
                    ...
                    state.scrollY = (1 - Mathf.InverseLerp(inputPosMin, inputPosMax, GetMousePos().y - state.dragScrollOffset)) * (contentHeight - scrollViewArea.Height);
                }
                else if (contentOverflow > 1)
                {
                    ...
                    bool mouseOverScrollView = MouseInBounds(scrollableBounds_ss.centre, scrollableBounds_ss.size);
                ...
                Vis.Quad(scrollbar_ss.centre, scrollbar_ss.size, scrollBarCol);
            }
        ...
        public static void DrawText(string text, FontType font, float fontSize, Vector2 pos, Anchor anchor, Color col)
        {
            if (IsRendering)
            {
                Vis.Text(font, text, fontSize * scale, UIToScreenSpace(pos), anchor, col);
            }

            TextRenderer.BoundingBox bounds = Vis.CalculateTextBounds(text.AsSpan(), font, fontSize, pos, anchor);
        ...
        public static InputFieldState InputField(UIHandle id, InputFieldTheme theme, Vector2 pos, Vector2 size, string defaultText, Anchor anchor, float textPad, Func<string, bool> validation = null, bool forceFocus = false)
        {
            ...
            if (IsRendering)
            {
                ...
                Vis.Quad(ss.centre, ss.size, theme.bgCol);

                // Focus input
                bool mouseInBounds = MouseInBounds(ss.centre, ss.size);
                ...
                using (CreateMaskScope(centre, size))
                {
                    ...
                    Vis.Text(theme.font, displayString, fontSize_ss, textCentreLeft_ss, Anchor.TextCentreLeft, textCol);

                    if (Application.isPlaying)
                    {
                        Vector2 boundsSizeUpToCaret = Vis.CalculateTextBoundsSize(displayString.AsSpan(0, state.cursorBeforeCharIndex), theme.fontSize, theme.font);

                        // Draw selection box
                        if (state.isSelecting)
                        {
                            Vector2 boundsSizeUpToSelect = Vis.CalculateTextBoundsSize(displayString.AsSpan(0, state.selectionStartIndex), theme.fontSize, theme.font);
            ...
            int CharIndexBeforeMouse(float textLeft)
            {
                ...
                float textBoundsWidth = Vis.CalculateTextBoundsSize(state.text, theme.fontSize, theme.font).x;
                float textRight = textLeft + textBoundsWidth * scale;
                float t = Mathf.InverseLerp(textLeft, textRight, GetMousePos().x);
        ...
        public static Vis.ID ReservePanel() => Vis.ReserveQuad();

        public static void ModifyPanel(Vis.ID id, Vector2 pos, Vector2 size, Color col, Anchor anchor = Anchor.Centre)
        {
            Vector2 centre = CalculateCentre(pos, size, anchor);
            (Vector2 centre, Vector2 size) ss = UIToScreenSpace(centre, size);
            Vis.ModifyQuad(id, ss.centre, ss.size, col);
            OnFinishedDrawingUIElement(centre, size);
        }

        public static void ModifyPanel(Vis.ID id, Bounds2D bounds, Color col)
        {
        ...
        public static void DrawPanel(Vector2 pos, Vector2 size, Color col, Anchor anchor = Anchor.Centre)
        {
            ...
            {
                (Vector2 centre, Vector2 size) ss = UIToScreenSpace(centre, size);
                Vis.Quad(ss.centre, ss.size, col);
            }
        ...
        public static void DrawLine(Vector2 a, Vector2 b, float thickness, Color col)
        {
            if (IsRendering)
            {
                Vis.Line(UIToScreenSpace(a), UIToScreenSpace(b), thickness * scale, col);
            }
        }
        ...
        public static void DrawCircle(Vector2 pos, float radius, Color col, Anchor anchor = Anchor.Centre)
        {
            ...
            {
                (Vector2 centre, Vector2 size) ss = UIToScreenSpace(centre, size);
                Vis.Point(ss.centre, ss.size.x, col);
            }
        ...
        public static Color DrawColourPicker(UIHandle id, Vector2 pos, float width, Anchor anchor = Anchor.Centre)
        {
            ...
            if (IsRendering)
            {
                ...
                Vis.SatValQuad(satVal_ss.centre, satVal_ss.size, state.hue);
                Vis.HueQuad(hue_ss.centre, hue_ss.size);
                ...
                Vis.Quad(hueHandlePos_ss, hueHandleSize_ss, Color.white);

                // Hue mouse input
                bool mouseOverHueBounds = MouseInBounds(hue_ss.centre, hue_ss.size);
                bool mouseOverHueHandle = MouseInBounds(hueHandlePos_ss, hueHandleSize_ss);
                ...
                {
                    state.hue = Remap01(hueBottomY_ss, hueTopY_ss, GetMousePos().y);
                }
                ...
                Vis.Point(satValHandlePos_ss, satValHandleRadiusOutline_ss, handleOutlineCol);
                Vis.Point(satValHandlePos_ss, satValHandleRadius_ss, colRgb);

                // Sat-val mouse input
                bool mouseOverSatValBounds = MouseInBounds(satVal_ss.centre, satVal_ss.size);
                ...
                {
                    state.sat = Remap01(satValBottomLeft_ss.x, satValTopRight_ss.x, GetMousePos().x);
                    state.val = Remap01(satValBottomLeft_ss.y, satValTopRight_ss.y, GetMousePos().y);
                }
        ...
        public static bool Button(string text, ButtonTheme theme, Vector2 pos, Vector2 size, bool enabled, bool fitTextX, bool fitTextY, Anchor anchor = Anchor.Centre, bool leftAlignText = false, float textOffsetX = 0, bool ignoreInputs = false)
        {
            ...
            {
                float minSizeX = Vis.CalculateTextBoundsSize(text.AsSpan(), theme.fontSize, theme.font).x;
                float minSizeY = Vis.CalculateTextBoundsSize(MString.AsSpan(), theme.fontSize, theme.font).y;
            ...
            if (IsRendering)
            {
                ...
                // --- Handle interaction ---
                bool mouseInsideMask = Vis.IsPointInsideActiveMask(GetMousePos()) && !ignoreInputs;
                bool mouseOver = mouseInsideMask && MouseInBounds(ss.centre, ss.size);
                ...
                Vis.Quad(ss.centre, ss.size, buttonCol);
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    Vis.Text(theme.font, text, fontSize_ss, textPos, textAnchor, textCol);
                }
            }
        ...
        public static bool MouseInsideBounds(Bounds2D uiBounds)
        {
            ...
            return MouseInBounds(ss.centre, ss.size);
        }
        ...
        public static ButtonState Button_Stateful(UIHandle id, string text, ButtonTheme theme, Vector2 pos, Vector2 size, bool enabled, bool fitTextX, bool fitTextY, Anchor anchor)
        {
            ...
            if (IsRendering)
            {
                ...
                // --- Handle interation ---
                bool mouseInsideMask = Vis.IsPointInsideActiveMask(GetMousePos());
                bool mouseOver = mouseInsideMask && MouseInBounds(ss.centre, ss.size);
                bool mouseIsDown = InputHelper.IsMouseHeld(MouseButton.Left);
                if (InputHelper.IsMouseDownThisFrame(MouseButton.Left))
                {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    Vis.Text(theme.font, text, fontSize_ss, ss.centre, Anchor.Centre, textCol);
                }
            }
        ...
        public static void DrawToggle(UIHandle id, Vector2 pos, float size, CheckboxTheme theme, Anchor anchor = Anchor.Centre)
        {
            ...
            if (IsRendering)
            {
                ...
                if (MouseInBounds(ss.centre, ss.size))
                {
                    if (InputHelper.IsMouseDownThisFrame(MouseButton.Left))
                    {
                        state = !state;
                ...
                if (!state)
                {
                    ...
                    Vis.Line(ss.centre - Vector2.one * crossScale, ss.centre + Vector2.one * crossScale, thickness, theme.tickCol);
                    Vis.Line(ss.centre + new Vector2(1, -1) * crossScale, ss.centre + new Vector2(-1, 1) * crossScale, thickness, theme.tickCol);
                }
            }
        ...
        {
            Vis.Quad(CalculateCentre(canvasBottomLeft, canvasSize, Anchor.BottomLeft), canvasSize, col);
        }
        ...
        public static void DrawLetterboxes()
        {
            ...
            if (canvasBottomLeft.x > 0)
            {
                ...
                Vis.Quad(CalculateCentre(Vector2.zero, size, Anchor.BottomLeft), size, col);
            }

            if (canvasTopRight.x < screenSize.x)
            {
                Vector2 size = new(screenSize.x - canvasTopRight.x, screenSize.y);
                Vis.Quad(CalculateCentre(Vector2.right * canvasTopRight.x, size, Anchor.BottomLeft), size, col);
            }
            ...
            if (canvasTopRight.y < screenSize.y)
            {
                ...
                Vis.Quad(CalculateCentre(Vector2.up * canvasTopRight.y, size, Anchor.BottomLeft), size, col);
            }
        }
