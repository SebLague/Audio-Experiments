...
// This class contains some helper functions to make life a little easier working with compute shaders (Very work-in-progress!)
    ...
    public static class ComputeHelper
    {
        ...
        public static void SetBuffers<T>(ComputeShader cs, T kernel, Dictionary<ComputeBuffer, string> nameLookup, params ComputeBuffer[] buffers) where T : Enum
        {
            SetBuffers(cs, Convert.ToInt32(kernel), nameLookup, buffers);
        }
        ...
        public static void CreateRenderTexture(ref RenderTexture texture, int width, int height, GraphicsFormat format = RGBA_SFloat)
        {
            CreateRenderTexture(ref texture, width, height, defaultFilterMode, format);
        }
        ...
        public static void CreateTexture2D(ref Texture2D texture, int width, int height, GraphicsFormat format = RGBA_SFloat, FilterMode filter = defaultFilterMode, TextureWrapMode wrapMode = TextureWrapMode.Clamp)
        {
            if (texture == null || texture.width != width || texture.height != height || texture.graphicsFormat != format)
            {
        ...
        public static void SetTexture(ComputeShader compute, Texture texture, string name, params int[] kernels)
        {
            ...
            {
                compute.SetTexture(kernel, name, texture);
            }
        }

        public static void SetTexture<T>(ComputeShader compute, Texture texture, string name, params T[] kernels) where T : Enum
        {
            foreach (T kernel in kernels)
            {
                int kernelIndex = System.Convert.ToInt32(kernel);
                compute.SetTexture(kernelIndex, name, texture);
            }
        }

        public static void SetTexture<T>(ComputeShader compute, Texture texture, int paramID, params T[] kernels) where T : Enum
        {
            foreach (T kernel in kernels)
            {
                int kernelIndex = System.Convert.ToInt32(kernel);
                compute.SetTexture(kernelIndex, paramID, texture);
            }
        }
