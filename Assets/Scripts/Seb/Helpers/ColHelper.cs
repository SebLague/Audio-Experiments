...
namespace Seb.Helpers
{
	public class ColHelper
	{
		...
		public static Color GenerateRandomColour(Random rng, float satMin = 0.2f, float valMin = 0.2f)
		{
			float hue = (float)rng.NextDouble();
			float sat = Mathf.Lerp(satMin, 1, (float)rng.NextDouble());
			float val = Mathf.Lerp(valMin, 1, (float)rng.NextDouble());
