...
namespace Seb.Helpers
{
    ...
    public static class Maths
    {
        ...
        public static bool CircleContainsPoint(Vector2 point, Vector2 circleCentre, float circleRadius)
        {
            float sqrDst = (point - circleCentre).sqrMagnitude;
            return sqrDst <= circleRadius * circleRadius;
        }
        ...
        {
            float elementWidth = GetElementSizeForSpacedLayout(numElements, size.x, spacing);
        ...
        public static float GetElementSizeForSpacedLayout(int numElements, float size, float spacing)
        {
            float spaceTotal = (numElements - 1) * spacing;
            float elementSize = (size - spaceTotal) / numElements;
            return elementSize;
        }
        ...
        // Returns point at time 't' (between 0 and 1) along Bézier curve defined by 4 points (anchor_1, control_1, control_2, anchor_2)
        public static Vector2 CubicBezier(Vector2 a1, Vector2 c1, Vector2 c2, Vector2 a2, float t)
        {
            t = Mathf.Clamp01(t);
            float u = 1 - t;
            return u * u * u * a1 + 3 * u * u * t * c1 + 3 * u * t * t * c2 + t * t * t * a2;
        }

        public static int CoordToFlatIndex(Vector2Int coord, int resolutionX)
        {
            return coord.y * resolutionX + coord.x;
        }

        public static Vector2Int FlatIndexToCoord(int flatIndex, int resolutionX)
        {
            int y = flatIndex / resolutionX;
            int x = flatIndex % resolutionX;
            return new Vector2Int(x, y);
        }

        // Convert 2D coordinate (on xz plane) to a 3D point with given height (y value)
        public static Vector3 CoordinateToWorldPos(Vector2Int coord, Vector2Int resolution, Vector2 worldSize, float height)
        {
            float u = resolution.x <= 1 ? 0.5f : coord.x / (resolution.x - 1f);
            float v = resolution.y <= 1 ? 0.5f : coord.y / (resolution.y - 1f);
            Vector2 uv = new Vector2(u, v);
            Vector2 worldPos2D = (uv - Vector2.one * 0.5f) * worldSize;
            return new Vector3(worldPos2D.x, height, worldPos2D.y);
        }

        // Convert world point to 2D coordinate on xz plane (world.y is ignored)
        public static Vector2Int WorldPosToCoordinate(Vector3 worldPos, Vector2Int resolution, Vector2 worldSize)
        {
            Vector2 worldPos2D = new Vector2(worldPos.x, worldPos.z);
            Vector2 coord = (worldPos2D / worldSize + Vector2.one * 0.5f) * (resolution - Vector2Int.one);
            return new Vector2Int((int)(coord.x + 0.5f), (int)(coord.y + 0.5));
        }
        ...
        public static void SampleYValsAtEqualIntervals(Vector2[] points, int count, ref float[] result)
        {
            Debug.Assert(count >= 2, "Expected count >= 2");
            ...
            {
                Vector2 a = points[pointIndex];
                Vector2 b = points[pointIndex + 1];

                if (a.x > b.x) throw new System.Exception("X vals not monotonically increasing");

                while (resultIndex < count)
                {
                    float nextX = Mathf.Lerp(points[0].x, points[^1].x, resultIndex / (count - 1f));
                    if (nextX > b.x) break;

                    float t = Mathf.InverseLerp(a.x, b.x, nextX);
                    result[resultIndex] = Mathf.Lerp(a.y, b.y, t);
                    resultIndex++;
                }
            }
        }
